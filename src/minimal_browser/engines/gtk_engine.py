...
class GtkWebEngine(WebEngine):
    ...
    def set_load_started_callback(self, callback: Callable[[], None]):
        ...
        if self._widget:
            self._widget.connect('load-changed', 
                lambda webview, event: callback() if event == WebKit.LoadEvent.STARTED else None)
    
    def set_load_progress_callback(self, callback: Callable[[int], None]):
        """Set callback for load progress"""
        if self._widget:
            self._widget.connect('notify::estimated-load-progress', 
                lambda webview, param: callback(int(webview.get_estimated_load_progress() * 100)))
    
    def set_load_finished_callback(self, callback: Callable[[bool], None]):
        """Set callback for load finished"""
        if self._widget:
            self._widget.connect('load-changed', 
                lambda webview, event: callback(True) if event == WebKit.LoadEvent.FINISHED else None)
