...
goals_module = import_module_direct(
    "minimal_browser.coordination.goals", str(src_dir / "coordination" / "goals.py")
)
...
def example_task_dependencies():
    ...
    task2 = Task(
        goal_id=goal_id, title="Create database tables", dependencies=[task1_id]
    )
    task2_id = manager.create_task(task2)

    task3 = Task(
        goal_id=goal_id, title="Implement API endpoints", dependencies=[task2_id]
    )
    task3_id = manager.create_task(task3)

    task4 = Task(goal_id=goal_id, title="Write unit tests", dependencies=[task3_id])
...
def example_multi_agent_workflow():
    ...
    for task in tasks:
        deps = (
            ", ".join([manager.get_task(d).title for d in task.dependencies])
